...
export async function generateMetadata({ params }: ProductDetailPageProps): Promise<Metadata> {
  ...
  if (!product) {
    return {
      ...
      description: "Material exclusivo não disponível.",
      robots: { index: false, follow: true },
    };
  }

  const canonicalPath = `/exclusivos/${slug}`;
...
export default async function ExclusivoDetalhePage({ params }: ProductDetailPageProps) {
  const { slug } = await params;
  const product = await loadProductBySlug(slug);
  ...
  const productLd = {
    ...
    url: absoluteUrl(`/exclusivos/${slug}`),
  ...
  const breadcrumbLd = buildBreadcrumbList([
    ...
    { name: product.title, path: `/exclusivos/${slug}` },
  ]);
      ...
      <article className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
        ...
        <div className="flex flex-col gap-5">
          ...
          <p className="text-xs text-muted-foreground pt-4 border-t">
            {product.clicks_count.toLocaleString("pt-BR")} pessoas já clicaram neste exclusivo.
          </p>
        </div>
      </article>
    </main>
