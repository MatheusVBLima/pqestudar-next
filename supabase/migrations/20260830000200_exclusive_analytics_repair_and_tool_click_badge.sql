-- Repairs the Exclusivos dashboard RPC for projects where the preceding
-- migration was copied manually, and introduces the public featured-tool
-- click badge without exposing individual analytics events.
...
VALUES (
  'featured_tools_click_badge',
  false,
  'Exibe nos cards em destaque a quantidade agregada de cliques recebidos.'
)
ON CONFLICT (key) DO NOTHING;

CREATE OR REPLACE FUNCTION public.featured_tool_click_counts_public()
RETURNS TABLE(tool_id UUID, click_count BIGINT)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT
    tool.id AS tool_id,
    COUNT(event.id)::BIGINT AS click_count
  FROM public.tools tool
  LEFT JOIN public.analytics_events event
    ON event.entity_type = 'tool'
   AND event.entity_id = tool.id::TEXT
   AND event.event_name = 'tool_card_click'
   AND event.actor_type IN ('public', 'anonymous')
  WHERE tool.is_visible = true
    AND tool.is_featured = true
  GROUP BY tool.id;
$$;

REVOKE ALL ON FUNCTION public.featured_tool_click_counts_public() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.featured_tool_click_counts_public() TO anon, authenticated;
